...
Which character was picked changes the ship's stats and, for some of them, the enemies themselves.
The whole subsystem — the cast, every perk and where it's applied, how to add or retune one, the
portrait pipeline, and player-created characters — is in **[characters.md](characters.md)**.
...
Player bullets already stopped dead on contact with layer 8 (`Bullet.OnBodyEntered`) — what obstacles didn't do until now was stop the player from *choosing* to fire at something on the other side of one in the first place. [`Targeting.HasObstacleBetween`](../Scripts/Util/Targeting.cs) is a raycast against layer 8 (same mechanism `Enemy.IsPathBlocked` already uses for enemy steering, applied here to targeting instead of movement), and `Player.FindNearestVisibleEnemy` — shared by the main gun (`OnFireCooldownTimeout`) and the Missile reward (`FindNearestEnemyInRange`) — sorts candidates by distance and returns the first one with a clear line to the player, skipping a closer-but-blocked enemy in favor of a farther visible one rather than wasting the shot. `Companion.FindNearestEnemy` (the drone) does the same check independently, raycasting from its own position rather than the player's. Laser, Onda de Choque, and the Pulso Nova ultimate are unaffected — they hit everything in range rather than aiming at one picked target, so there's no "choice" for a line-of-sight check to filter.
...
- The Heart reward (a single Legendary, shop-only) adds `+1` to `MaxLives` up to `MaxLivesCap = 4` and fully heals you. See [rewards.md](rewards.md#additive--full-heal-heart) — it used to exist at all four tiers under a more convoluted "max tracks best tier" rule, which stopped making sense once there was only one tier. The cap was lowered from 6 as part of closing the late-round "practically invincible" problem — see [difficulty-scaling.md](difficulty-scaling.md#survivability-catch-up) for why a smaller life pool matters there.
...
- HUD shows exactly 4 individual heart icons (`HeartIcon.cs`, filled = a life you have, dim outline = a life you've lost), one per point of `MaxLivesCap` — same pattern as the shield row's 4 icons for `MaxShieldChargesCap`. No text fallback: with the cap fixed at 4, a row of hearts always reads cleanly.
...
- `CooldownIcon` draws its own clock: a dark disc with a coloured rim and the ability's symbol, plus a dark pie sector covering the fraction still on cooldown, swept from 12 o'clock. No texture — plain `_Draw()`, same procedural-only approach as the rest of the game's visuals. 1 = just used (fully covered), 0 = ready (fully clear). Both the disc and the sweep are built on the block grid via `Juice.DrawPixelCircle`/`Juice.WedgePoints`, at **half** the project's `Juice.PixelSize` — at 36px this icon's radius is only ~16, and the full grid would leave it five blocks wide with no room for a readable glyph.
