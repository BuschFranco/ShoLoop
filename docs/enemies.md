...
`Scripts/Enemy/EnemyBullet.cs` reads the same multiplier in its own `_PhysicsProcess`, for the same live-read reason — a shot already in flight when Zona Lenta triggers slows down mid-flight, not just shots fired afterward. This used to be missing entirely: enemies slowed down but kept firing at full projectile speed, so the ultimate only ever did half its job. Contrast with knockback (`Enemy.ApplyKnockback`), which is deliberately kept *outside* this multiplier so Zona Lenta doesn't also weaken a shove — bullet travel speed doesn't have an equivalent reason to stay untouched, so it's included.
