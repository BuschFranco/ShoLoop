...
1. **Clears every enemy** (`foreach (Node in GetTree().GetNodesInGroup("enemies")) enemy.QueueFree()`) — the board is always empty at the start of the round-end shop, regardless of what was still alive (or mid-Split) when the round ended.
