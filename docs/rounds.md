...
4. Starts a 3-second "get ready" countdown (`_roundStartTimer`, `RoundStartDelay = 3f`, exposed as `GameManager.RoundStartTimeRemaining`/`IsRoundStarting`) — but does **not** unpause. `HUD._Process` checks `IsRoundStarting` first, ahead of the boss-round check, and shows the remaining seconds in a dedicated **`CountdownLabel`** — 72px, yellow, centered on screen — while the small top-bar timer just reads "Preparate...". The countdown originally lived in that corner label alone and was effectively invisible there.
5. **Stays paused for the whole countdown.** `_roundStartTimer` and `HUD` both run with `ProcessMode.Always` (the same "keeps working through `Pause()`" pattern the Shop/UpgradePicker already use) specifically so the countdown itself keeps ticking and the label keeps updating while everything else — enemies, the player, in-flight pickups — stays frozen. This used to unpause here instead, and it was a real bug: a leftover XP/coin pickup from the previous round (deliberately not swept, see above) could still be collected during those 3 "free" seconds, and if that crossed a level threshold it queued a brand-new level-up picker that popped up *after* the player had already watched the countdown start — reads as a dialog leaking in after the round had "already begun." Keeping the tree paused until gameplay is genuinely about to resume closes that gap.
6. When the countdown elapses, `BeginRoundAfterCountdown()` unpauses (`GameManager.Resume()`) — this, not step 4, is the actual moment live gameplay resumes — then branches on `IsBossRound` (`RoundNumber % 5 == 0`):
...
Before a boss round, `StartNextRound()` (step 4) already calls `DangerDirector.SetBossAlert(true)`, but `DangerOverlay`'s pulse is a plain `Tween` — deliberately **not** `ProcessMode.Always` (see that file: an alarm that kept strobing behind a paused modal would be both ugly and a photosensitivity risk) — so it stays frozen along with everything else until `Resume()` fires at the end of the countdown. Net effect: the alarm's fade-in/pulse now effectively starts right as the boss spawns rather than telegraphing for the 3 seconds beforehand, a minor trade-off against not letting anything else move during that window either.
...
- The spawner keeps running, but **only spawns Grunts, and roughly a fifth as many as a normal round would**. `ConfigureForBossRound` clears the board (any stragglers from the previous round were already cleared by `EndRound()` anyway), then sets `_forcedSpawnScene = EnemyScene` so `OnSpawnTimeout` bypasses `ChooseEnemyScene()` entirely, and scales the interval/burst/cap by `BossRoundIntervalMult`/`BossRoundCrowdMult`. The chaff is there to crowd the player's movement while the Boss remains the only real threat — and keeping it Common-only means the Boss stays the sole non-Common on the field, so the recap's "especiales" count still reads exactly 1.
  ...
  **Only the first `BossRoundRewardedSpawns` (20) chaff pay out.** A boss round has no time limit — it ends solely on the Boss's death — so the concurrent cap bounds nothing over time: every chaff kill frees a slot instantly, and at round 20's steady rate a two-minute fight could spawn over a thousand Grunts. Stalling the Boss was free, unlimited XP.

  Capping the *count* was the first attempt and it was worse: the arena went eerily quiet once the budget ran dry. So spawning now continues indefinitely and only the payout is capped — past the quota, chaff arrives with `Enemy.GrantsRewards = false`, meaning no XP, no Score, no coins and no XP/coin drops. Hearts are the sole exception, at a deliberately generous 7% (`UnrewardedHeartDropChance`) rather than the 0.15% the late-round taper would otherwise give, so a drawn-out boss fight is winnable by attrition instead of being pure attrition against you. The flag propagates through `Split()` too, so an unrewarded parent can't launder XP through its children.

  Unrewarded kills still increment `EnemiesKilled` — they happened, and the HUD total should say so — but they show no `+N` score popup, since that's gated on the XP actually granted.
