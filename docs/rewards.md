...
| **Crit** | additive + cap | Rolled through the single shared `Player.ApplyCrit()` by **every** damage source — basic shots (per bullet, not per volley, so a spread can crit on some lines and not others), Laser, Orbit Blades, Missiles, and the Companion drone — not just the basic gun. Crit hits tint gold (bullets, missiles, drone shots, laser beams); orbit blades skip the tint to avoid flicker. See [player.md](player.md#crit-golpe-crítico) for the per-weapon breakdown. |
| **Knockback** | tier-bucketed | Reuses the same `Enemy.ApplyKnockback` the orbit blades use; shoves along the bullet's travel direction. Bosses are immune to all knockback (`Enemy.ApplyKnockback` no-ops for `EnemyCategory.Boss`), so a swarm of blades/bullets can't stunlock a boss's movement. |
...
Crit and Pierce also feed `Player.GetOffensivePower()` as real damage multipliers — `critMult` now scales the Laser/Missile/Orbit DPS terms too, not just the gun's. Without that a crit/pierce-heavy build would read as far weaker than it plays and would dodge the [adaptive difficulty](difficulty-scaling.md#adaptive-difficulty-difficultybalancer) correction entirely.
...
**Owning and swapping**: `Player.EquippedUltimate` (nullable `UltimateKind`) holds at most one at a time. `IsRewardUseless(Ultimate)` is true only if you already have that *exact* kind — a different kind is always buyable even with one equipped, and buying it **replaces** your current one (`ApplyUpgrade`'s `Ultimate` case always does `EquippedUltimate = upgrade.Ultimate; UltimateCooldownRemaining = 0f;` — ready immediately, first pick or swap alike, not a reset charge bar).

**Cooldown, not a charge meter**: the Ultimate used to fill from Score/XP; it's now a flat time-based cooldown, `Player.UltimateCooldownRemaining` counted down every physics frame, no Score/kill involvement at all. `Player.UltimateCooldownDuration` reads a `RoundCurve(10f, -0.65f, 3.5f, 10f)` off `GameManager.RoundNumber` — 10s at round 1, shrinking linearly to 3.5s by round 11 and holding there. A freshly-equipped Ultimate starts at `UltimateCooldownRemaining = 0`, i.e. usable the instant you pick it, not after an initial wait.
