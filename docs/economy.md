...
- **Score tracks *base* XP, not boosted XP.** The **Sabiduría** reward (`UpgradeType.XpBonus`, Common +20% / Epic +50%, up to +100%) multiplies the XP fed to `AddXp` but **not** the value passed to `AddScore`. This is the one place Score and XP deliberately diverge, for two reasons: Score is persisted as a high score, so letting a reward choice inflate it would make runs incomparable; and `AddScore` also charges the Ultimate meter, so multiplying there would quietly turn Sabiduría into an Ultimate-charge reward too.
...
- Also drives the **Ultimate charge meter** — see [rewards.md](rewards.md#ultimates) — via `GameManager.AddScore` calling `Player.AddUltimateProgress(amount)` on every gain.
